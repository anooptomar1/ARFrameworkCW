...
class VirtualObject: SCNReferenceNode {
    ...
    var modelName: String {
        return "Test"//referenceURL.lastPathComponent
    }
...
extension VirtualObject {
    ...
    static let availableObjects: [VirtualObject] = {
        //ARLogging.getZipFileFromUrl("teacup")
        //ARLogging.unzipObject(endpoint: "teacup")
        let asset = ARLogging.getObject()
        //let url = asset.url
        //let asset = MDLAsset()
        let object = asset.object(at: 0)
        let node = SCNNode(mdlObject: object)
        let vObject = VirtualObject(mdlObject: object)
        return [vObject]
